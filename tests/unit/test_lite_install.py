...
def test_release_docs_pin_current_images_and_document_quick_default():
    ...
    assert "quick manifest now defaults to `1.0.2`" in english
    assert "快速安装清单现在默认使用 `1.0.2`" in chinese
    assert "still defaults to `1.0.1` until post-publication promotion" not in english
