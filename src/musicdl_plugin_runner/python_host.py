...
from musicdl.contracts.plugin import HttpAction, PluginInvocation, PluginResponse, PluginStep
...
def _step_from_value(inv, value):
    if isinstance(value, dict) and set(value) == {"action"}:
        try: return PluginStep(action=HttpAction.model_validate(value["action"]))
        except Exception: return _error(inv,"plugin_error","plugin execution failed")
...
def _run(inv):
    try:
        ...
        return _step_from_value(inv, value)
