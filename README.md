...
The quick Compose project name defaults to `musicdl`. To change the host port, set `MUSICDL_PORT` before running Compose. To select a different published application image, set `MUSICDL_IMAGE_TAG`; the quick manifest now defaults to `1.0.3`. Both application images use the same version tag.
