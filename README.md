...
`/admin` serves the login page, the source and Bot controls, aggregated dependency health, and the event and audit logs. The session and CSRF cookies are marked `Secure`, so the panel must be reached over HTTPS through a TLS-terminating reverse proxy. Logins are rate limited, mutating routes require the double-submit CSRF token, and until the default `admin`/`password` credentials are changed every panel route except `/admin/` and `/admin/change-credentials` answers 403. The portal lists the sources the runtime registry assembled at startup. Bot definitions are owned here as well: `POST /admin/bots` creates one, `PATCH /admin/bots/{id}` edits its username, command template, priority, timeout, and enabled flag, and `DELETE /admin/bots/{id}` removes it. A Bot is deliberately not published as a source, because a toggle there would not control anything. Stored changes and credentials are written to `MUSICDL_ADMIN__STATE_PATH` and reloaded on start, so an edited source, an edited Bot, and a changed password all survive a restart, and a stored entry takes precedence over the value the runtime publishes again on boot. Compose sets that path to `/data/app/admin-state.json` on the `musicdl-app-data` volume. A state file that is unreadable or carries an unsupported version stops startup instead of silently reverting the password to the default, and a failed write rolls the in-memory change back.
