param([ValidateSet('all','compose','proxy','wecom-callback','telegram-session-isolation','media-integrity','plugin-security','admin-auth','compose-recovery')][string]$Gate='all',[switch]$DryRun,[switch]$SelfTest)
